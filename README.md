...
從 [Releases](https://github.com/ilzfor/quota-resume/releases) 下載 `quota-resume-0.2.0.zip`，完整解壓後，將其中的 `quota-resume` 資料夾交給 Codex，輸入：
...
**週用量已使用達 70% 時停止**，可按任務自訂。例如：「使用 $quota-resume，將這個任務的週用量停止門檻設為 70%。」達門檻會停用排程，需再次要求啟用才接續，不會下週自動重啟。檢查發生在排程開始與工作階段之間，不能強制中斷正在執行的工作或限制其他任務的用量。

升級 0.1.0 後，請在已有排程的原任務要求設定門檻，讓 Codex 更新其保存的 prompt；只重裝外掛不會修改既有排程。
...
使用 Python 3 執行 `python package_plugin.py`，產生 `dist/quota-resume-0.2.0.zip`。26 項測試與打包只使用 Python 標準函式庫。若修改此外掛，請重新打包並依 Codex 本機外掛的更新流程重裝。
